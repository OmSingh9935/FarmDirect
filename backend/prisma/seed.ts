...
async function main() {
  ...
  for (let i = 0; i < 42; i++) {
    ...
    let photoKey: keyof typeof samplePhotos = 'tomato';
    if (cropName.includes('Onion')) photoKey = 'onion';
    else if (cropName.includes('Potato')) photoKey = 'potato';
    else if (cropName.includes('Rice')) photoKey = 'rice';
    else if (cropName.includes('Mango')) photoKey = 'mango';
    else if (cropName.includes('Chilli')) photoKey = 'chilli';
    else if (cropName.includes('Wheat')) photoKey = 'wheat';

    // Varied grades
    const grades: ('A' | 'B' | 'C')[] = ['A', 'A', 'B', 'A', 'B', 'C', 'A'];
